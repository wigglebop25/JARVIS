...
import { Cpu, User, FileText, X, Brain, ChevronDown, ChevronRight, Wrench } from 'lucide-react';
...
const ToolCallBlock = ({ toolCalls }: { toolCalls: ToolCall[] }) => {
  ...
  return (
    <div className="my-2 space-y-1">
      {toolCalls.map((tc, idx) => (
        <div key={idx} className="border border-theme-border/30 bg-theme-surface-2/10 rounded-lg overflow-hidden text-[12px]">
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="w-full flex items-center gap-2 px-3 py-1.5 bg-theme-surface-2/20 text-theme-accent/60 hover:text-theme-accent select-none font-mono text-[10px] uppercase tracking-wider font-semibold transition-colors cursor-pointer"
          >
